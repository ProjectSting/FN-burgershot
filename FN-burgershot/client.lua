...
AddEventHandler("FN-burgershot:congelador", function()
    if onDuty == true then
        ...
        exports['mythic_notify']:DoHudText('error', 'You need to be on duty!')
    end
end)
    	...
    	QBCore.Functions.TriggerCallback('FN-burgershot:server:get:ingrmoneyshot', function(HasItems)
    		if HasItems then
				...
				QBCore.Functions.Progressbar("fazer_moneyshot", "Making the moneyshot...", 4000, false, true, {
                    ...
                    exports['mythic_notify']:DoHudText('Success ', 'Fizeste um moneyshot')
				end, function()
					TriggerEvent('inventory:client:busy:status', false)
					Working = false
				end)
			else
                exports['mythic_notify']:DoHudText('error ', "You don't have the ingredients!")
			end
		end)
	else
        exports['mythic_notify']:DoHudText('error', 'You need to be on duty!')
    ...
    if onDuty then
    	QBCore.Functions.TriggerCallback('FN-burgershot:server:get:ingrbatata', function(HasItems)
    		if HasItems then
                ...
                exports['mythic_notify']:DoHudText('error', "You don't have all the ingredients!")
			end
		end)
	else
        exports['mythic_notify']:DoHudText('error', 'You need to be on duty!')
    ...
    if onDuty then
    	QBCore.Functions.TriggerCallback('FN-burgershot:server:get:ingrtomate', function(HasItems)
    		if HasItems then
                ...
                exports['mythic_notify']:DoHudText('error', 'You lack tomatoes or a kitchen knife')
			end
		end)
	else
        exports['mythic_notify']:DoHudText('error', 'You need to be on duty!')
    ...
    if onDuty then
    	QBCore.Functions.TriggerCallback('FN-burgershot:server:get:ingrbatata_cortar', function(HasItems)
    		if HasItems then
                ...
                exports['mythic_notify']:DoHudText('error', 'You lack potatoes or a kitchen knife!')
			end
		end)
	else
        exports['mythic_notify']:DoHudText('error', 'You need to be on duty!')
    ...
    if onDuty then
    	QBCore.Functions.TriggerCallback('FN-burgershot:server:get:ingrbatata_lavar', function(HasItems)
    		if HasItems then
                ...
                exports['mythic_notify']:DoHudText('error', "You don't have potatoes to wash")
			end
		end)
	else
        exports['mythic_notify']:DoHudText('error', 'You need to be on duty!')
    ...
    if onDuty then
    	QBCore.Functions.TriggerCallback('FN-burgershot:server:get:copobebida', function(HasItems)
    		if HasItems then
                ...
                exports['mythic_notify']:DoHudText('error', "You don't have a cup!")
			end
		end)
	else
        exports['mythic_notify']:DoHudText('error', 'You need to be on duty!')
